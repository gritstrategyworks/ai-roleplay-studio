...
- 公開リンクとQRコードによるアプリ共有
- 無料版向けネイティブ収益化枠（Premium案内、直接スポンサー、成果報酬リンクに対応）
- スマートフォン対応、PWAキャッシュ
...
無料版の収益化内容は `public/monetization.json` で管理します。標準はStripe Premiumへの自社案内です。直接スポンサーまたは成果報酬リンクに切り替える場合は、`kind` を `sponsor` または `affiliate`、CTAの `action` を `external` とし、`href` に発行済みの計測URLを設定してください。外部リンクにはアプリ側で `rel="sponsored"` が付与されます。
