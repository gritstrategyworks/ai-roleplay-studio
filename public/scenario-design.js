...
((root) => {
  ...
  const CATEGORY_COPY = Object.freeze({
    ...
    newhire: {
      publicLabel: '事前に分かっている業務情報',
      publicHelp: '依頼内容や進捗など、会話前に新入社員が把握している事実だけを入力します。上司・先輩の判断基準はAIが非公開で保持します。',
      publicPlaceholder: '例：顧客別実績資料は3社分を確認済みだが、1社の回答待ちで提出が1日遅れる見込み',
      directionLabel: '上司・先輩が重視する点の方向性',
      directionPlaceholder: '例：遅れよりも、影響範囲と次の対応が分からないことを懸念している',
      truthLabel: '上司・先輩の本音・判断基準',
      truthPlaceholder: '例：完成度より、16時までの再見積もりとリスク共有を期待している',
      conditionsLabel: '確認したい事項・譲れない条件',
      conditionsPlaceholder: '例：結論、事実、影響、対応案、判断してほしい点が整理されていること',
    }
  });
  ...
  const AUTO_PROFILES = Object.freeze({
    ...
    newhire: [
      { truth: '遅れそのものより、影響範囲と次の対応が分からないことを懸念している', conditions: '結論、確認できた事実、影響、対応案、判断してほしい点が整理されていれば判断できる' },
      { truth: '指示の期限だけが伝わり、資料を会議で使う目的と完成イメージが共有できていない', conditions: '目的、A4一枚、数字と課題を含めること、途中確認の時刻を復唱してほしい' },
      { truth: '電話の用件よりも、相手の会社名・氏名・連絡先・折り返し期限が正確に伝わるかを確認したい', conditions: '要点を復唱し、本日15時までの折り返しを明確に案内してほしい' },
      { truth: '完成度より、遅れやミスを早く共有して優先順位を組み替えられることを重視している', conditions: '現状、期限への見込み、懸念、次の行動、次回報告時刻を共有してほしい' },
    ]
  });
  ...
  const DISCOVERY_SIGNALS = Object.freeze({
    ...
    newhire: [
      ['結論・目的', /(結論|報告|連絡|相談|用件|目的)/],
      ['事実・現状', /(事実|現在|確認|進捗|状況|見込み)/],
      ['期限・優先順位', /(期限|いつまで|優先|時|日)/],
      ['確認・完成イメージ', /(理解|認識|完成|形式|合っていますか|復唱)/],
      ['次の行動・相談', /(次に|対応|相談|判断|途中報告|振り返)/],
    ]
  });
  ...
  function buildPublicBriefing(category, config = {}) {
    ...
    if (category === 'newhire') return `${scene}の練習。利用者は新入社員として、${product}に関する状況を上司・先輩へ伝える。必要な確認事項は対話で整理する。`;
