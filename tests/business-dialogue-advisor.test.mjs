...
test('dedicated avatar and mobile advisor layout are production assets', async () => {
  ...
  assert.match(auth, /advisor\.js\?v=1\.64/);
  assert.match(serviceWorker, /advisor\.js\?v=1\.64/);
...
test('operator email and shared VOICEVOX lecture credit remain visible', () => {
  assert.doesNotMatch(html, /Grit Strategy Works/);
  assert.match(html, /運営者：gritstrategyworks@gmail\.com/);
  assert.match(html, /講義音声：VOICEVOX:\u56db\u56fd\u3081\u305f\u3093/);
  assert.doesNotMatch(html, /新入社員(?:向け)?講義の音声/);
